/**
 * Code adopted from https://github.com/naamor/reveal.js-tableofcontents
 * 
...
window.RevealTableOfContents = window.RevealTableOfContents || {
  id: 'RevealTableOfContents',
  init: function(deck) {
      initTableOfContents(deck);
  }
};

const initTableOfContents = function(Reveal) { 
    ...
    function generateList() {
        ...
        for (counter; counter < slides.length; counter++) {
            ...
            if (title !== undefined) {
                ...
                let href_text = `#/${title.replace(/ +/g, '-').toLowerCase()}`;
                a.href = href_text;
                a.innerText = title
                li.appendChild(a)
